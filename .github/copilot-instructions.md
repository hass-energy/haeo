...
HAEO is a Home Assistant custom integration that optimizes energy usage across battery storage, grid import/export, loads, and generators using linear programming. The integration provides real-time optimization based on energy prices, forecasts, and system constraints.
...
- **Sensors**: Expose optimization results and entity states to Home Assistant

### Configuration Schema

Users configure entities (batteries, grids, loads, generators) and connections between them through the Home Assistant UI. The integration validates this configuration and builds an optimization network.

## HAEO-Specific Development Guidelines

### Energy Model Development

- **Entity Classes**: Inherit from base `Entity` class, implement power/energy variables
- **Network Building**: Use `Network.add()` method with entity type strings
- **Optimization**: Linear programming constraints only - keep models simple for solver efficiency
- **Units**: Power in Watts (W), Energy in Watt-hours (Wh), Time in seconds

### Integration Development

- **Sensor Creation**: Use generic property detection instead of entity type checking
- **Configuration**: Use Voluptuous schemas for validation, store in `ConfigEntry.data`
- **Coordinator**: Update interval based on optimization complexity, not user input
- **Error Handling**: Catch optimization failures gracefully, set status appropriately
...
This project uses **uv** for fast, reliable Python package management and development workflows.
...
```bash
# Run tests (most common during development)
uv run pytest                          # All tests
uv run pytest tests/test_model.py      # Specific test file
...
uv add pytest-mock --dev             # Development dependency

# Install project in development mode
uv pip install -e .                   # Editable install for testing
...
### uv Advantages for HAEO Development

- **Speed**: 10-100x faster than pip for dependency resolution
- **Reliability**: Consistent dependency resolution with `uv.lock`
- **Simplicity**: Single tool for project management, dependencies, and running commands
- **Compatibility**: Drop-in replacement for pip/poetry workflows
- **Modern**: Built for Python 3.12+ with latest ecosystem practices
