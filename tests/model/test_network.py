...
def test_connect_source_is_connection() -> None:
    ...
    network.add({"element_type": ELEMENT_TYPE_NODE, "name": "grid1", "is_sink": False, "is_source": True})
    network.add(
        {"element_type": ELEMENT_TYPE_CONNECTION, "name": "conn1", "source": "battery1", "target": "grid1"}
    )
...
def test_network_optimize_validates_before_running() -> None:
    ...
    network.add(
        {"element_type": ELEMENT_TYPE_CONNECTION, "name": "conn1", "source": "node1", "target": "node2"}
    )

    # Connect conn2 to conn1 (invalid)
    network.add(
        {"element_type": ELEMENT_TYPE_CONNECTION, "name": "conn2", "source": "conn1", "target": "node2"}
    )
...
def test_add_battery_balance_connection() -> None:
    ...
    network.add({"element_type": ELEMENT_TYPE_BATTERY, "name": "upper_section", "capacity": 10.0, "initial_charge": 5.0})
    network.add({"element_type": ELEMENT_TYPE_BATTERY, "name": "lower_section", "capacity": 10.0, "initial_charge": 5.0})
