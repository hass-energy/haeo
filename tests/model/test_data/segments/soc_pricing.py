...
SCENARIOS: list[SegmentScenario] = [
    {
        ...
        "spec": {
            ...
            "lower_energy_threshold": np.array([3.0, 3.0]),
            "upper_energy_threshold": np.array([8.0, 8.0]),
            "lower_energy_price": np.array([0.5, 0.5]),
            "upper_energy_price": np.array([0.2, 0.2]),
        },
        "periods": np.array([1.0, 1.0]),
        "inputs": {"minimize_cost": True},
        "expected_outputs": {
            "lower_energy_slack": (1.0, 0.0),
            "upper_energy_slack": (0.0, 1.0),
            "objective_value": 0.7,
        },
...
ERROR_SCENARIOS: list[SegmentErrorScenario] = [
    ...
    {
        "description": "SOC pricing requires lower threshold with price",
        "factory": SocPricingSegment,
        "spec": {"segment_type": "soc_pricing", "lower_energy_price": np.array([0.1])},
        "periods": np.array([1.0]),
        "error": ValueError,
        "match": "lower_energy_threshold is required when lower_energy_price is set",
        "endpoint_factory": _battery_endpoints_free,
    },
    {
        "description": "SOC pricing requires upper threshold with price",
        "factory": SocPricingSegment,
        "spec": {"segment_type": "soc_pricing", "upper_energy_price": np.array([0.1])},
        "periods": np.array([1.0]),
        "error": ValueError,
        "match": "upper_energy_threshold is required when upper_energy_price is set",
        "endpoint_factory": _battery_endpoints_free,
    },
]
