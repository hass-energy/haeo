...
SCENARIOS: list[SegmentScenario] = [
    {
        ...
        "expected_outputs": {
            "undercharge_slack": (1.0, 0.0),
            "overcharge_slack": (0.0, 1.0),
            "objective_value": 0.7,
        },
...
ERROR_SCENARIOS: list[SegmentErrorScenario] = [
    ...
    {
        "description": "SOC pricing requires undercharge threshold with price",
        "factory": SocPricingSegment,
        "spec": {"segment_type": "soc_pricing", "undercharge_price": np.array([0.1])},
        "periods": np.array([1.0]),
        "error": ValueError,
        "match": "undercharge_threshold is required when undercharge_price is set",
        "endpoint_factory": _battery_endpoints_free,
    },
    {
        "description": "SOC pricing requires overcharge threshold with price",
        "factory": SocPricingSegment,
        "spec": {"segment_type": "soc_pricing", "overcharge_price": np.array([0.1])},
        "periods": np.array([1.0]),
        "error": ValueError,
        "match": "overcharge_threshold is required when overcharge_price is set",
        "endpoint_factory": _battery_endpoints_free,
    },
]
