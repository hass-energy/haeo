...
├── environment.json  # HA version, HAEO version, timezone, and the captured timestamps
...
- **environment.json**: HA version, HAEO version, timezone, and the captured `diagnostic_request_time`, `diagnostic_target_time`, `optimization_start_time`, `optimization_end_time`, and `horizon_start` - user editable for testing
...
The `environment.json` `optimization_start_time` is used as the freeze time for the test, and `outputs.json` is compared with actual optimization results.
