...
async def test_schema_rejects_empty_forecast_list(hass: HomeAssistant, hub_entry: MockConfigEntry) -> None:
    """Schema validation should reject empty forecast entity list."""
    ...
    # Attempt to validate input with empty forecast list
    with pytest.raises(vol.MultipleInvalid) as exc_info:
        schema({CONF_NAME: "Test Solar", CONF_CONNECTION: "TestNode", CONF_FORECAST: []})

    assert CONF_FORECAST in str(exc_info.value)
...
async def test_user_step_with_configurable_shows_values_form(
    ...
    # Submit step 1 with configurable entity selected for forecast
    step1_input = {
        CONF_NAME: "Test Solar",
        CONF_CONNECTION: "TestNode",
        CONF_FORECAST: [TEST_CONFIGURABLE_ENTITY_ID],
...
async def test_reconfigure_empty_forecast_shows_error(
    ...
    """Reconfigure with empty forecast should show error."""
    add_participant(hass, hub_entry, "TestNode", node.ELEMENT_TYPE)
...
async def test_reconfigure_values_step_updates_entry(
    ...
    assert update_kwargs["data"][CONF_FORECAST] == 8.0
