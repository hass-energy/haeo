...
async def async_setup_entry(
    ...
    for subentry in config_entry.subentries.values():
        ...
        # Create number entities for numeric input fields
        for field_info in input_fields:
            if field_info.entity_type != InputEntityType.NUMBER:
                continue
    ...
    if entities:
        ...
        _LOGGER.debug("Created %d input number entities", len(entities))
    else:
        _LOGGER.debug("No input number entities created for entry %s", config_entry.entry_id)
