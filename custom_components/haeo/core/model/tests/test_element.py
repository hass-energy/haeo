...
def test_element_power_balance_shadow_price_has_n_periods_values(solver: Highs) -> None:
    ...
    runs. For a Battery (both production and consumption), decomposition
    constraints live in ``_element_power_decomposition``. The
    ``element_power_balance`` output must therefore contain exactly n_periods
    balance duals, not decomposition duals mixed in.
    """
    n = 2
    periods = np.array([1.0] * n)
    battery = Battery(
